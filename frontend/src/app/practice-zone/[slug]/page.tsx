...
export default function ChallengePage() {
    ...
    const handlePredict = async (direction: "UP" | "DOWN") => {
        ...
        setPhase("revealing");
        try {
            const predictionResult = await submitPrediction(slug, direction);
    ...
    return (
        <DashboardLayout xp={result ? result.xp_earned : 0}>
            ...
            <motion.button
                ...
                onClick={() => router.push("/practice-zone")}
                className="flex items-center gap-2 text-sm text-[#6b7280] hover:text-[#6B1D3A] mb-4 transition-colors cursor-pointer font-medium"
            >
            ...
            {phase === "loading" && !error && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                    ...
                    <p className="text-sm text-[#9ca3af] font-medium">Loading challenge...</p>
                </motion.div>
            )}
